...
const completeOnboarding = async () => {
  ...
  try {
    ...
    const { oauth_url } = await $fetch<OauthResponse>(
      "http://localhost:8000/square/oauth/start/",
      {
        method: "GET",
        headers: {
          Authorization: `Token ${token.value}`,
    ...
    pendingOauthUrl.value = oauth_url;
    showSquareModal.value = true;
  } catch (error) {
...
const continueToSquare = () => {
  window.location.href = pendingOauthUrl.value;
};

const cancelSquareFlow = () => {
  showSquareModal.value = false;
};
  ...
  >
    ...
    >
      <div class="bg-white p-6 rounded shadow-lg max-w-md w-full">
        <h2 class="text-lg font-bold mb-2">Connect Your Square Account</h2>
        <p class="mb-4">
          You’ll now be redirected to Square to securely connect your POS. This
          allows you to sync your CrossPerks perks directly into your register.
        </p>
        <div class="flex justify-end gap-3">
          <button
            @click="cancelSquareFlow"
            class="text-gray-600 hover:underline"
          >
            Cancel
          </button>
          <button
            @click="continueToSquare"
